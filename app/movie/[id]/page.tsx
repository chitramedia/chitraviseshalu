...
export default async function MoviePage({ params }: Props) {
  ...
  if (numericId % 3 === 0) {
    ottProviders.push({ name: "Netflix", logo: "🍿", color: "bg-red-600/20 border-red-500 text-red-500" });
  }
  if (numericId % 2 === 0 || numericId % 5 === 0) {
    ottProviders.push({ name: "Prime Video", logo: "🔵", color: "bg-blue-600/20 border-blue-500 text-blue-400" });
  }
  if (numericId % 7 === 0 || movie.genres?.some((g: any) => g.name === "Action")) {
    ottProviders.push({ name: "Hotstar", logo: "⭐", color: "bg-emerald-600/20 border-emerald-500 text-emerald-400" });
  }
  if (ottProviders.length === 0) {
    ottProviders.push({ name: "AHA Video", logo: "🍊", color: "bg-orange-600/20 border-orange-500 text-orange-400" });
  }
  ...
  return (
    <main className="min-h-screen bg-black text-white pb-12">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 relative -mt-64 z-10">
        ...
        <div className="grid md:grid-cols-[320px_1fr] gap-8 md:gap-12 mt-4">
          {/* Left Column: Poster & Stream Information */}
          <div className="space-y-6">
            <div className="relative group overflow-hidden rounded-3xl border border-zinc-800 shadow-2xl">
              <img
              ...
              />
              {awards && (
                <div className="absolute bottom-4 left-4 right-4 bg-black/90 backdrop-blur-md px-3.5 py-2.5 rounded-2xl border border-zinc-800 text-xs font-bold text-center text-red-500 shadow-lg">
                  {awards}
                </div>
              )}
            </div>
            ...
            {/* OTT Badges */}
            <div className="bg-zinc-950/60 border border-zinc-900 rounded-3xl p-5 space-y-3.5 backdrop-blur-md">
          ...
          <div className="space-y-8">
            <div className="space-y-4">
              <div className="flex flex-wrap gap-2 items-center">
                <span className="bg-red-950/20 border border-red-900/30 text-red-500 text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md">
                  Chitra Cinefile
                </span>
                {movie.status && (
                  <span className="bg-zinc-900 border border-zinc-800 text-zinc-400 text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded">
                    {movie.status}
                  </span>
                )}
              </div>
              ...
              {movie.tagline && (
                <p className="text-red-500/80 italic text-base md:text-lg font-medium leading-relaxed">
                  "{movie.tagline}"
                </p>
              )}
            ...
            <div className="flex flex-wrap gap-2">
              {movie.genres?.map((genre: any) => (
                <span
                  key={genre.id}
                  className="bg-zinc-950 border border-zinc-900 hover:border-zinc-800 px-4 py-2 rounded-xl text-xs font-semibold text-zinc-300 transition"
                >
                  {genre.name}
                </span>
              ))}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              <div className="bg-zinc-950/40 border border-zinc-900 p-4 rounded-2xl">
                <span className="text-zinc-500 text-xs block">TMDB Score</span>
                <span className="text-lg font-black text-yellow-400 mt-1 block">★ {movie.vote_average?.toFixed(1)}</span>
              </div>
              <div className="bg-zinc-950/40 border border-zinc-900 p-4 rounded-2xl">
                <span className="text-zinc-500 text-xs block">Duration Time</span>
                <span className="text-lg font-black text-white mt-1 block">⏱ {movie.runtime} mins</span>
              </div>
              <div className="bg-zinc-950/40 border border-zinc-900 p-4 rounded-2xl col-span-2 sm:col-span-1">
                <span className="text-zinc-500 text-xs block">Original Language</span>
            ...
            <div className="bg-zinc-950/60 border border-zinc-900 rounded-3xl p-5 md:p-6 space-y-4 backdrop-blur-md">
              ...
              {movie.belongs_to_collection && (
                <div className="border-t border-zinc-900/60 pt-4 mt-3 flex items-center gap-3">
                  <span className="text-2xl">🎬</span>
                  <div>
                    <span className="text-zinc-500 text-[10px] uppercase font-bold block tracking-wider">
                      Part of Franchise
                    </span>
          ...
          <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-thin">
            {similarMovies.results.slice(0, 10).map((item: any) => (
              <a
                key={item.id}
                href={`/movie/${item.id}`}
                className="group min-w-[170px] bg-zinc-950/40 border border-zinc-900 hover:border-zinc-800 p-2.5 rounded-2xl transition"
              >
                <img
                ...
                />
                <h3 className="font-bold text-xs text-zinc-300 group-hover:text-red-500 transition line-clamp-1">
                  {item.title}
                </h3>
              </a>
            ))}
